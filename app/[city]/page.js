...
function formatCity(c) {
  const cityWithSpaces = c.replace(/%20/g, ' ');
  const words = cityWithSpaces.split(' ');
...
export default async function Home({params}) {
  ...
  const res = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=a1e186a8588b478124305174d32d7503`, {
    method: 'GET',
    header: {
  ...
  return (
    < div className = { styles.container}>
      <div className={styles.textContainer}>
        <h1 className={styles.title}>{formatCity(city)}</h1>
